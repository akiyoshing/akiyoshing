const ringData = {
  "ティファニー": [
    { carat: 0.3, name: "クラシックソリティア", image: "img/tiffany1.jpg", desc: "伝統的なソリティア。" }
  ],
  "カルティエ": [
    { carat: 0.3, name: "ソリテール", image: "img/cartier1.jpg", desc: "シンプルで上品なデザイン。" }
  ],
  "ロイヤルアッシャー": [
    { carat: 0.3, name: "アッシャーカットA", image: "img/ra1.jpg", desc: "王室にも愛される王道デザイン。" },
    { carat: 0.3, name: "アッシャーカットB", image: "img/ra2.jpg", desc: "サイドに小粒ダイヤ入り。" },
    { carat: 0.3, name: "アッシャーカットC", image: "img/ra3.jpg", desc: "華やかなアーム。" }
  ],
  "ラザール": [
    { carat: 0.4, name: "ユニオン", image: "img/laz1.jpg", desc: "輝きを最大限に引き出すデザインで、あふれる光を指先に纏って" },
    { carat: 0.4, name: "トンプソン", image: "img/laz2.jpg", desc: "サイドのダイヤがメインのダイヤを輝かす" },
    { carat: 0.4, name: "ローズランド", image: "img/laz3.jpg", desc: "5石の装飾で、華やかかつ優美なデザイン" },
    { carat: 0.4, name: "アーヴィング", image: "img/laz4.jpg", desc: "端正でスタンダードな「約束」の証" }
  ],
  "ブリリアンス＋": [
    { carat: 0.8, name: "ソリティア", image: "img/bp1.jpg", desc: "大粒で迫力のある印象" },
    { carat: 0.8, name: "サイドストーン", image: "img/bp2.jpg", desc: "サイドの石でメインの石を引き立てる" },
    { carat: 0.8, name: "パフェ", image: "img/bp3.jpg", desc: "細身アームにを6石のダイヤで煌びやかに" }
  ],
  "NextDiamond": [
    { carat: 1.01, name: "ラボグロウンダイヤ", image: "img/nd1.jpg", desc: "大きなダイヤが目を引く、シンプルなデザイン(注意：ラボダイヤ)" }
  ]
};
